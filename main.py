...
class Application:
    def __init__(self, master_window=None):
        ...
        self.containerInterface.grid(
            row=0, column=1, sticky=tk.E+tk.W+tk.N+tk.S)

        self.containerTxtBUSCA = tk.Frame(self.containerInterface)
        self.containerTxtBUSCA.grid(
            sticky=tk.E+tk.W+tk.N+tk.S, padx=10, pady=10)
        ...
        self.containerListBoxTextArea.grid(
            sticky=tk.E+tk.W+tk.N+tk.S, padx=10, pady=10)
        ...
        self.listboxCall = tk.Listbox(
            self.containerSelecao, width=60, height=10, selectmode=tk.SINGLE)
        self.listboxCall.bind("<Double-Button-1>", self.curseletCall)
        self.scrollbarCall = tk.Scrollbar(
            self.containerSelecao, orient="vertical")

        self.lblBUSCA = tk.Label(
            self.containerTxtBUSCA_1, height=1, text="BUSCAR:", font=self.fonte, width=8)
        self.lblBUSCA.grid(row=0, column=0, sticky=tk.E+tk.W+tk.N+tk.S)

        self.txtBUSCA = tk.Entry(
            self.containerTxtBUSCA_2, width=34, font=self.fonte)
        self.txtBUSCA.grid(row=0, column=0, sticky=tk.E+tk.W+tk.N+tk.S)

        self.rbtnCALLID = tk.Radiobutton(
            self.containerbtnBUSCA, text="CALLID", variable=self.tipoBusca, value="CALLID")
        self.rbtnCALLID.grid(row=0, column=0, sticky=tk.E+tk.W+tk.N+tk.S)

        self.rbtnTELEFONE = tk.Radiobutton(
            self.containerbtnBUSCA, text="TELEFONE", variable=self.tipoBusca, value="TELEFONE")
        self.rbtnTELEFONE.grid(row=0, column=1, sticky=tk.E+tk.W+tk.N+tk.S)

        self.btnBUSCA = tk.Button(
            self.containerbtnBUSCA, text="BUSCAR", width=10, font=self.fonte, command=self.buscaCall)
        self.btnBUSCA.grid(row=0, column=2, sticky=tk.E+tk.W+tk.N+tk.S)

        self.lblInfo = tk.Label(self.containerlblInfo, justify=["left"], font=self.fonte,
                                width=100, height=10, textvariable=self.lblInfoVar)
        self.lblInfo.grid(row=0, column=0, sticky=tk.E+tk.W+tk.N+tk.S)

        self.listbox = tk.Listbox(
            self.containerListBoxTextArea, selectmode=tk.EXTENDED)
        self.listbox.bind("<Double-Button-1>", self.curselet)
        self.scrollbar = tk.Scrollbar(
            self.containerListBoxTextArea, orient="vertical")
        ...
        if escolha == "TELEFONE":
            for bilhete in self.log.bilhetes:
                if chamada in bilhete.TO or chamada in bilhete.FROM:
                    ...
                    for linha in bilhete.conteudo:
                        self.listbox.insert(
                            self.listbox.size(), str(linha).replace("\n", ""))
                    self.listbox.insert(self.listbox.size(), "\n")
                bilhete = Pacote()
        else:
            for bilhete in self.log.bilhetes:
    ...
    def openFiles(self):
        ...
        for id in self.log.callid:
            count = count + 1
            self.listboxCall.insert(
                self.listbox.size(), str(id).replace("\n", ""))
        ...
        if len(savefilename) > 1:
            ...
            try:
                ...
                messagebox.showerror(
                    message="Problemas para salvar no arquivo informado")
        else:
            messagebox.showwarning(
                message="Favor colocar o nome do arquivo final")
